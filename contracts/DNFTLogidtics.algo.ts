...
export class DNFTLogidtics extends Contract {
  ...
  nft_trasferFrom(_from: Address, _to: Address, _tokenId: uint64): void {
    ...
    if (
      this.txn.sender === this.tokens(_tokenId).value.owner ||
      this.tokens(_tokenId).value.control === this.txn.sender ||
  ...
  createParcel(
    ...
    const parcel: parcelStatus = { currentLocation: _initialLocation, status: _status, lastUpdated: _lastUpdated };
